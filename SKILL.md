...
5. When reporting back, start from the report's first screen (two verdicts, key numbers, reconciliation and sensitivity status, overview figure); point the user to `report.html` for reading and quote the "看图要点" of any figure that shows a problem. If a reconciliation check fails or the sensitivity table shows a flipped conclusion, say so first.
