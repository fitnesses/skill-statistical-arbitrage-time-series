...
Paths such as `scripts/run_statarb.py` and `requirements.txt` are relative to **this skill's directory**; call the script by its absolute path and keep configs, snapshots and run outputs in the user's workspace (e.g. `./statarb_runs/<name>/`), not inside the skill. The zeus endpoint comes from the environment variables `ZEUS_MCP_URL` and `ZEUS_MCP_TOKEN`; if they are unset, ask the user to set them (never print or store the token in files).
