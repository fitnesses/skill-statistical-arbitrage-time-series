...
> 行情数据只来自 **zeus MCP**（期货日线 `fut_daily`，以及待 zeus 实现的 `fut_basic`/`fut_settle`/`ft_limit`，接口见 [`references/zeus-mcp-interface.md`](../references/zeus-mcp-interface.md)）：内置脚本用 `--fetch` 调 MCP，把响应原样存成不可变快照，再离线回放；不接第三方数据源、不直连数据库。统计检验一律用 `statsmodels`（必需依赖，缺失时脚本直接报错，不做近似），保证 p 值真实。
...
flowchart LR
    ...
    F --> G["💰 真实合约整手回测<br/>次日成交·手续费·滑点·保证金·涨跌停"]
...
| 🔁 **回测与偏差控制** | 研究口径：价差对数收益 · 样本内外隔离 · 近似成本 ｜ **交易可行性：真实合约·整手·次日成交·逐腿手续费/滑点·多空保证金·涨跌停一腿不成交则两腿都不成交·流动性标记·压力情景** | 样本外还成立吗？换成真实合约和整手、扣费后还剩多少？保证金收益率多少？ |
...
flowchart TD
    ...
    subgraph FUT["🧾 期货可行性规则"]
        ...
        F3["🟡 成交顺延（一腿封板/无行情）· 流动性不足"]
        F4["🟡 涨跌停为推断（缺 ft_limit）· 合约参数用配置假设（缺 fut_basic/fut_settle）"]
...
│   └── zeus-mcp-interface.md      # 🔌 zeus MCP 工具约定（fut_daily 已有；fut_basic/fut_settle/ft_limit 待实现）
├── scripts/
...
| 💸 成本即证据 | 真实合约、整手、次日成交；逐腿手续费（费率/每手，开平各计）+ 滑点（跳）+ 多空保证金；涨跌停一腿不成交则两腿都不成交；给毛/净、压力情景与保证金收益率 |
