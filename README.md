...
统计套利与时间序列建模 Agent Skill（中文优先），面向国内商品期货的跨期与跨品种价差。数据只经 zeus MCP 获取并存为不可变快照，可离线复跑；覆盖按时点安全的合约映射与换月、训练窗 ADF/KPSS/Engle-Granger、价差建模与均值回归、真实合约整手回测（逐腿手续费、滑点、保证金、涨跌停与流动性约束）以及稳健性风险。统计证据与交易可行性分开判定。
